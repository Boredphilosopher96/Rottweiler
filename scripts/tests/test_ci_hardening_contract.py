...
class CiHardeningContractTests(unittest.TestCase):
    ...
    def test_release_preflight_reuses_protected_performance_and_stays_non_publishing(
        ...
        self.assertEqual(preflight.count("continue-on-error: true"), 4)
        ...
        self.assertIn("id: channel_transition", preflight)
        self.assertIn("scripts/check-release-channel-advance.py", preflight)
        self.assertIn("steps.channel_transition.outcome", preflight)
        self.assertIn("release-channel-transition.json", preflight)
