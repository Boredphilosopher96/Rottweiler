...
fn create_requires_exact_correlated_singleton() {
    ...
    assert_eq!(
        created_session(listed("create", vec![descriptor("owned")]), &request).unwrap(),
        Some(SessionSummary {
            id: "owned".into(),
            state: "driver".into()
        })
...
fn authorized_ids_have_no_duplicate_or_path_selectors() {
    ...
    validate_authorized(&mut exact).unwrap();
    assert_eq!(exact, ["a", "z"]);
...
fn fixed_readonly_catalog_and_arguments_fit_declared_construction() {
    let descriptors = tool_descriptors(super::super::read_only_tools().unwrap()).unwrap();
    assert_eq!(descriptors.len(), 4);
    let input = json!({"path": "file.txt", "offset": 2, "limit": 10});
    let (arguments, permission) = tool_arguments(input.clone()).unwrap();
...
async fn lost_cpu_waiter_keeps_source_until_physical_work_retires() {
    ...
    let slot =
        McpResponseSlot::new(rw_mcp::McpResponseLimits::new(WORKING_BYTES).unwrap()).unwrap();
