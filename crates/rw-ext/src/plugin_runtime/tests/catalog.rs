...
async fn structured_rpc_support_requires_explicit_cached_catalog_evidence() {
    ...
    provider
        .discover_models()
        .await
        .expect("declared structured support");
    assert!(provider.supports_structured_output("structured"));
    assert!(!provider.supports_structured_output("other"));
    let missing = catalog_adapter(json!({"models":[{"id":"missing","capabilities":{
    ...
    assert_eq!(
        missing
            .discover_models()
            .await
            .expect_err("required support field")
            .kind,
