//! Host-approved bootstrap code, pinned independently of a mutable installation path.
...
pub struct HelperArtifactIdentity {
...
/// Immutable authority to execute the host's internal sandbox bootstrap.
#[derive(Clone, Debug)]
pub struct SandboxHelper(Arc<OwnedHelper>);
#[derive(Debug)]
struct OwnedHelper {
...
impl SandboxHelper {
    ...
    pub fn from_artifact(approved: &HelperArtifactIdentity) -> Result<Self, SandboxError> {
        if approved.bytes == 0
            || approved.bytes > MAX_HELPER_BYTES
        ...
        #[cfg(not(target_os = "linux"))]
        let launch_path = directory.path().join("rw-sandbox-helper");
        #[cfg(not(target_os = "linux"))]
