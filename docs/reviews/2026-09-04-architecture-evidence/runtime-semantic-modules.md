...
The repository-wide cap still reports four core and five TUI files. The production raw-network check now honors Rust-enforced test-only file attributes and follows the Linux sandbox loopback owner. Files merely named `tests.rs` remain checked. Fifteen checker tests pass. Combined core372/runtime264 tests and strict core/runtime/provider/sandbox Clippy pass after indexed recovery, core contract extraction and client diagnostics integration. Runtime presentation remains in the explicitly named `cli_output` module until the A08 caller migration; this split does not claim A08 completion.
