...
| --- | --- | --- | --- |
| C01 | P1: Restore a trustworthy main branch and complete gate ownership | Candidate and post-merge CI verified | PR54 candidate6c3a4ae passed every gate in run33930648431. Main3ef57e9 has the identical tree. Ruleset19590466 now requires CI required from GitHub Actions; every unrelated rule is unchanged. Post-merge run33931348188 completed successfully at main3ef57e9. |
...
| C08 | P1: Every failed gate must leave usable evidence | Hosted failure artifact verified; abrupt runner-loss qualification pending | Downloaded failed macOS rust-tests artifact from run33929264078:exit101,actual merge-checkout SHA,runner image/architecture,elapsed time and bounded log tail containing the shutdown failure. Wrapper/cancellation tests pass. Abrupt runner-loss behavior remains unexercised. |
...
| A12 | P2: Spend limits observe usage but do not reserve in-flight cost | Transactional authority verified; runtime and provider adoption underway | Exact durable call/attempt receipts, atomic cross-session admission, retained ambiguous charges and fixed-depth time totals. Eleven new store tests and the complete store suite pass; [evidence](2026-09-04-architecture-evidence/provider-budget-admission.md). |
| A13 | P2: Checkpoint capture can allocate without a file-size budget | Aggregate store limits verified; runtime ownership integration underway | Required shared operation allowance, cancellable chunked scans, bounded metadata/Git output, and owned lock/result lifecycle. Store suite: 198 passed, 2 ignored; strict all-target clippy passed. Runtime cancellation/panic checks await the required engine settlement API integration. [Evidence](2026-09-04-architecture-evidence/checkpoint-ownership-and-bounds.md). |
| A14 | P2: Workspace intelligence lacks an aggregate cache and freshness contract | Shared owner integrated and tested; controlled performance qualification pending | WorkspaceIndexPool shares canonical-root/trust-key indexes, bounds aggregate memory and scan work, reconciles external edits, and moves blocking scans off the actor. Seventeen index tests and complete tools/runtime suites passed before subsequent integration. |
...
| A20 | P2: Performance diagnosis lacks stage-level ownership | Core stage tracing integrated; fault injection and overhead qualification pending | TRACE spans identify context, turn, tool, compaction and provider latency using IDs and counts without recording arguments. Journal/UI stages and controlled disabled-overhead measurements remain in scope. |
...
| A22 | P2: WASM hooks pay process and compilation costs per invocation | Source checkpoint verified; capacity qualification pending | Application-owned bounded worker reuse, fresh stores, cache identity and owned cleanup; [evidence](2026-09-04-architecture-evidence/wasm-worker-reuse.md). Two-worker default remains provisional. |
