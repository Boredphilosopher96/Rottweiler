...
Composer text and attachments are editable client state. Asynchronous clipboard, pasted-path and external-editor results are bound to the initiating draft generation and session scope. Replacing or submitting the draft retires those input operations; destroying the component prevents their results and errors from mutating another surface. One external editor invocation is admitted per composer. `ComposerDraftStore` admits active, suspended and in-flight drafts under one 32 MiB retained-size budget and 256-entry limit. It transfers a submitted draft into a pending reservation; settlement releases that charge or merges the exact failed input into its originating draft. Reset retires the destination while retaining pending allocation charges until settlement. Different attachment versions remain distinct. A recovered draft may exceed the per-message attachment count, but submission refuses until the user removes enough attachments. Drafts are not evicted to admit other data.

Native textarea growth is checked before insertion. Prompt recall retains at most 256 entries and 1 MiB. Native undo operations accumulate a conservative edit charge; a 4 MiB checkpoint resets the add-buffer and undo records while preserving text, cursor and selection. These retained-data charges are not a process-RSS measurement. Attachment size admission counts exact JSON bytes without allocating the encoded body; immutable attachment sizes are memoized with weak keys. Secret prompts and process-bound interactions are excluded from renderer handoff; they do not enter persisted conversation history merely because a component captures its view state.
