...
- **Provider OAuth is configuration-driven and native-app safe**: authorization/token endpoints, public client id, scopes, and token credential references are accepted only from trusted user scope. The CLI uses an external browser URL, an ephemeral `127.0.0.1` callback, fresh injected OS entropy for state and PKCE verifier, `S256` only, exact state/redirect validation, a bounded callback timeout, and a no-redirect token client. Provider-issued refresh-token rotations must be durably stored before the corresponding access token is exposed; storage failure fails closed. Authorization codes, verifiers, access tokens, refresh tokens, token error bodies, and proxy passwords are never rendered in diagnostics. There are no built-in undocumented consumer endpoints or shared native-app secrets.
