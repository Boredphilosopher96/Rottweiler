...
- **One private credential file**: all non-environment credentials live as logical keys inside one versioned Rottweiler-owned file (mode 0600 on Unix). Startup, diagnostics, provider discovery, and authentication read that file without invoking an operating-system credential store.
...
- Interactive approval choices are **Ask** (`strict`), **Auto** (`auto-safe`), and **Off** (`yolo`); wire and configuration values remain stable. Ask prompts for edits and commands outside the hardened safe-list. Auto allows read-only actions and reversible workspace file edits, and asks before other actions, including unsandboxed commands. Explicit deny rules and Discuss/Plan restrictions still refuse actions. Launch-fixed headless `auto-safe` retains its unattended behavior: unknown or risky actions are denied without prompting. Off skips routine prompts while preserving explicit denies and the sandbox.
- Hardened read-only commands include ordinary `grep` and `rg` searches and a restricted `find` expression grammar, alongside `cat`, `ls`, `bat`, `git status`, and `git diff`. Search executables resolve through audited system/package locations; quoted arguments retain their boundaries. Ripgrep runs without configuration and cannot request preprocessing, hostname commands, or decompression helpers. Find execution, deletion, and file-output actions require approval. Build/package commands such as `cargo` and `npm` execute project code and remain outside this safe-list.
- Remembered session/project approvals bind the exact invocation and executable identity. The approval dock offers **Review a permission rule…** for an explicit user-authored pattern; opening it neither approves the waiting invocation nor installs a blanket `tool(*)` grant. Interactive Auto honors reviewed allow rules as well as its automatic safe actions, while retaining deny precedence and separate network/unsandboxed authority. Rules and exact approvals have different authority: no prefix glob is inferred automatically from a command, because basename-based rules do not retain the executable, environment, and workspace binding of exact approvals.
...
- **Linux**: Landlock ABI V3 scopes filesystem access; fresh user, PID, and network namespaces isolate the command. A separate seccomp syscall policy enforces the network floor. Calls without network authority cannot create network sockets. Calls with bounded egress authority use a loopback relay inside the network namespace to reach the supervised domain-policy proxy over a Unix socket. Plain seccomp-bpf does not inspect destination addresses. Required Landlock or namespace support being unavailable refuses sandbox launch; there is no automatic weaker backend substitution.
...
Single-process plugins reserve their trusted supervisor and sandbox execution processes before launch (two units on macOS; three on Linux including the namespace launcher). A private mode-0700 namespace under `/tmp` keeps the Unix socket address within its fixed path capacity independently of `TMPDIR`. Its close-on-exec Unix socket must identify the spawned supervisor before the host grants execution; its pathname is removed at that handoff. Parent cancellation or process death closes the lifeline. The supervisor kills its actual unreaped child, then waits for physical retirement. Linux's `--kill-child` namespace launcher and the supervisor's child-subreaper role also retire and reap the namespace children; macOS denies plugin process creation. A crashed supervisor remains the host's unreaped process-group anchor: the host observes exit without reaping, terminates that group, then reaps. Successful settlement additionally requires the supervisor’s fixed retirement receipt and group absence. A missing receipt remains unsettled even when the original group is absent: macOS process-creation denial does not prevent a plugin from changing its own process group. No forced cancellation kills a still-running supervisor before its child cleanup. Immutable executable, helper, and process credits remain owned until this proof; an unavailable proof does not become successful shutdown.
...
- A native plugin attempting direct workspace reads/writes, child creation, or outbound networking → denied by its code-only sandbox. Approved filesystem/HTTP effects succeed only through the exact active host invocation scope.
