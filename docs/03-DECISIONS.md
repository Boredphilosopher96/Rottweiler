...
Projection version 3 includes a first-class turn-summary row sourced only from
...
Finished child rows retain a bounded touched-file count and an optional canonical
`SubagentDiff` content selector. Opening changes uses the ordinary byte-paged
content channel; it does not create another client artifact store or raw replay.
...
`rw-macos-bootstrap` owns the Mach FFI and every returned pointer and send right. Its running-image owner queries `proc_pidinfo(PROC_PIDREGIONPATHINFO)` at a statically linked code address, validates the returned executable mapping, and exposes its kernel vnode identity. Bootstrap capture compares that identity to the opened executable before retaining a verified private snapshot; a replaced installation path cannot impersonate the running image. One process-owned snapshot is reused only after each requested descriptor passes kernel vnode and original size, mode, modification-time, and change-time revalidation. It is the audited exception to the workspace unsafe-code prohibition. `rw-sandbox` selects it for the single-process policy; the host application is unaffected.
