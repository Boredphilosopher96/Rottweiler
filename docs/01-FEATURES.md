...
- **Built-in `/` commands**: `/help`, `/mode`, `/model`, `/compact`, `/cost`, `/context`, `/init`, `/deep-init`, `/agents`, `/mcp`, `/permissions`, `/resume`, `/fork`, `/rewind`, `/review`, `/add-dir`, `/theme`, `/config`, `/memory`, `/trust`, `/exit`. `/permissions mode default|strict|auto-safe|yolo` changes the local interactive session policy without restarting the app; launch-fixed headless and remote-strict policies cannot be weakened from the TUI. Transcript export is the offline CLI command `rw export <session>`.
...
- **Unified message IR**: one internal representation for messages, tool calls, thinking blocks, images, citations. Providers are adapters: Anthropic Messages and OpenAI Chat/Responses plus OpenAI-compatible endpoints (OpenRouter/Ollama/vLLM/anything). Compatible Chat adapters normalize legacy `function_call`, modern parallel `tool_calls`, string or block-array content, refusals, partial streaming arguments, and provider-specific terminal reasons without selecting a wire shape from a model id or a local file. Gemini is supported in v1 through Google's official OpenAI-compatible endpoint; a native Gemini-wire adapter is post-v1 unless that compatibility surface proves insufficient.
...
`read`, `write`, `edit` (string replace: exact match first, whitespace-normalized fallback; ambiguous matches fail with candidate locations listed — never guess), `multi-edit`, `grep` (ripgrep engine), `glob`, `ls`, `bash` (sandboxed, see 05-SECURITY), `webfetch` (URL → markdown; size-capped, injection-dampened, no routine permission prompt, SSRF-guarded, honors the proxy config), `websearch` **[gap]** (web search: provider-native search where the model supports it, else a configured search API; results through the same egress policy), `todo` (task list the model maintains), `ask_user`, `spawn_agent` (§6). All tools:
