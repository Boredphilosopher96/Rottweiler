...
- **Auth**: API keys resolve from an optional `api_key_env` first, then `api_key_credential` in the OS keychain (warned mode-0600 fallback). `rw auth set-key <provider>` reads only from a hidden TTY prompt—never argv or an environment handoff—and stores under the configured identifier or `providers.<name>.api_key`. Providers that document native-app subscription auth can be configured with their authorization endpoint, token endpoint, public client id, and scopes. `rw auth login <provider>` uses Authorization Code + PKCE (`S256`) with an ephemeral IP-literal loopback callback, validates state, and stores access/refresh tokens in the same credential manager. Rottweiler does not embed or guess consumer OAuth endpoints/client ids. Keys and OAuth codes/tokens never enter session logs. AWS Bedrock / Google Vertex adapters (auth-divergent enterprise routes) are **explicitly post-v1** — the OpenAI-compatible adapter is the v1 escape hatch via a gateway.
