...
Child display publishers coalesce at most one queued observation per admitted child, with one 8 MiB allocation allowance shared by preview construction, observer delivery, and queue slots. Producers acquire encoded/parser scratch before serialization, encode once under the 256 KiB wire cap, and admit the structural JSON decode bound before building a value. Only engine events and existing JSON values are accepted as preview inputs; the pinned JSON decoder preserves finite-number round-trips. The required preview carrier retains that credit through every observer and until the live event bus admits it; callbacks cannot receive an unowned JSON copy. Coalescing releases discarded preview bodies before their credit, while queue identity metadata uses the same allowance. Both tool-launched and hosted children use the same slot semantics. Saturated delivery marks the next canonical source fence; it never waits for display capacity before settling child effects. Hosted lifecycle records and observations enter the same actor queue, with durable terminal acknowledgement releasing the active progress binding.
