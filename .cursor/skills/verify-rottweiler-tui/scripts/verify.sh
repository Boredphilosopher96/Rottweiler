...
case "$command_name" in
  doctor)
    ...
    ;;
  conversation|command-palette|approval|tools|theme-browser)
  ...
  cleanup)
    ...
    ;;
  *)
    printf 'usage: %s {doctor|conversation|command-palette|approval|tools|theme-browser|smoke|cleanup} [evidence-dir]\n       %s present <scenario.ansi>\n' "$0" "$0" >&2
    exit 2
    ;;
esac
