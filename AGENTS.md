...
- Before Rust tests that start native plugins, run `ROTTWEILER_TEST_SANDBOX_HELPER_RECEIPT="$(python3 scripts/build-test-helper.py)" && export ROTTWEILER_TEST_SANDBOX_HELPER_RECEIPT` using the same worktree target. The script atomically publishes the helper and a tiny non-shipped ownership fixture, with separate flat identity receipts in one bundle bound to both hashes. Test harness binaries cannot dispatch sandbox worker entrypoints; tests consume the explicit receipts, never search target outputs or compile prerequisites. Real Bun conformance remains a separate requirement.
