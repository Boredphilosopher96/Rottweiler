...
test("native panel actions are keyboard reachable, exact-revision commands, and survive closing until settlement", async () => {
  ...
  try {
    app.openMcpPicker()
    app.mcpBrowser.selectById("mcp.panels")
    app.mcpBrowser.activateSelected()
    await waitForHistory(setup, () => app.picker.items.some(item => item.label === "Inspection result"))
...
test("disconnect retires displayed panel strings and catalog picker references before another view opens", async () => {
  ...
  try {
    ...
    expect(app.picker.items).toHaveLength(0)
    await open()
    app.picker.activateSelected()
