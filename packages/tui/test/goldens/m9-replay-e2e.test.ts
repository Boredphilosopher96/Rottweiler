...
test("authenticated observer reads semantic pages without replaying lifetime events", async () => {
  ...
  const engine = new AuthenticatedMockEngine([
    ...
    return { type: "read", outcome: { type: "accepted" }, events: [{ type: "transcript_page_ready",
      ...
      result: await source.page({ sessionId: command.session_id, scope: command.scope }, command.read, new AbortController().signal, { admit: () => {} }),
  ...
  try {
    ...
    expect(frame).toContain("● Read PROJECT.md")
    expect(frame).toMatch(/● Read PROJECT\.md +✓/)
    expect(frame).not.toContain("Open content")
