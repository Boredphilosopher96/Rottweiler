...
function fixture(text: string) {
  ...
  const reader = { ...pages, content: async (_session: SessionReadTarget, read: import("../src/protocol").TranscriptContentRead) => {
...
test("timeline pages restore sources beyond the recent live window using the shared byte owner", async () => {
  ...
  await mounted.open(directSessionRead("s"))
...
test("source mismatch, stalled chunks, and cancellation release read reservations", async () => {
  for (const failure of ["source", "offset", "abort"] as const) {
    ...
    const broken = { ...reader, content: async (session: SessionReadTarget, read: import("../src/protocol").TranscriptContentRead) => {
