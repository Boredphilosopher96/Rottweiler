...
function liveTool(): ToolProjection {
  return {
    ...
    chunks: toolOutputBuffer([]), output: null, isError: false, callIndex: 0, timing: { kind: "unknown" }
  }
}
...
describe("semantic history navigation", () => {
  ...
  test("one invocation transfers from live output to its canonical row with expansion and selection intact", async () => {
    ...
    items.push(final)
    app.handleEvent({
      type: "tool_call_finished", meta: meta("3"), turn_id: "1", tool_call_id: tool.toolCallId,
