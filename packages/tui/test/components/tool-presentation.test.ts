...
describe("tool-presentation components", () => {
  ...
  test("renders bash commands and existing mutation diffs inline with syntax-aware renderables", async () => {
    ...
    // A one-line command is fully visible in the header; no second command block.
    expect(bashCard?.command).toBeNull()
    expect(bashCard?.header.plainText).toStartWith("● Bash cargo test --workspace")
    expect(editCard?.diff).toBeInstanceOf(DiffRenderable)
    expect(editCard?.header.plainText).toStartWith("● Edit src/main.rs  +1 −1")
    ...
    // The diff is the result of a successful edit; its summary fields are not repeated.
  ...
  test("caps inline diffs with stats and a review footer", async () => {
    ...
    expect(card.height).toBe((card.body.height ?? 0) + 1 + (card.diff?.height ?? 0) + 1)
    expect(card.header.plainText).toStartWith("● Edit src/large.rs  +26 −26")
  ...
  test("renders canonical display fields instead of protected model framing", async () => {
    ...
    expect(card?.header.plainText).toStartWith("● Diagnostics src/main.rs")
    card?.toggle()
    await setup.renderOnce()
