...
test("task state is committed independently of failed or transformed tool presentation", () => {
  ...
  state = reduce(state, { type: "tool_call_finished", payloads: [], presentation: null, meta: meta("3"), turn_id: "1", tool_call_id: "todo", invocation_id: "todo-1", output: { type: "structured", value: snapshot("presentation-only") }, is_error: false, call_index: 0 })
  expect(state.todos.snapshot).toEqual(snapshot("authoritative"))
  state = reduce(state, { type: "tool_call_finished", payloads: [], presentation: null, meta: meta("4"), turn_id: "1", tool_call_id: "failed", invocation_id: "failed", output: { type: "text", text: "Output hook failed after the state commit" }, is_error: true, call_index: 1 })
  expect(state.todos.snapshot).toEqual(snapshot("authoritative"))
})
...
test("task history has one snapshot and uses the same completed tool retention as every tool", () => {
  ...
  for (let index = 0; index < 300; index++) {
    ...
    state = reduce(state, { type: "tool_call_finished", payloads: [], presentation: null, meta: meta(String(++seq)), turn_id: String(index), tool_call_id: `todo-${index}`, invocation_id: `todo-${index}`, output: { type: "text", text: "done" }, is_error: false, call_index: 0 })
  }
