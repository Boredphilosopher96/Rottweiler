...
export const plugin = definePlugin({
  manifest: {
    ...
    capabilities: {
      commands: [{ name: "context-panel", description: "Inspect context and publish session state", allowed_tools: [] }],
      push: ["session/context_read", "session/control", "extension/state_read", "extension/state_commit", "ui/publish_panel"],
      ui: [{ surface: "panel", id: "context", title: "Context", fields: [{ kind: "text", id: "summary", label: "Summary", path: [{ step: "field", name: "summary" }] }], actions: [] }],
    },
  },
  handlers: {
    commands: {
      "context-panel": async (_params, { session, state, push }) => {
