...
6. **Provider-blind execution.** The engine speaks one internal message IR and providers remain adapters. A bounded, credential-free catalog may cross the boundary for explicit model/provider selection: display names, capabilities, availability, sanitized auth/reachability state, and supported auth interaction kind. Provider configuration, credential references or values, endpoints, proxy details, wire errors, and routing implementations stay inside `rw-providers`.
