...
6. **Provider-blind execution.** The engine speaks one internal message IR and providers remain adapters. Only bounded, credential-free provider display names may cross the boundary for explicit model/provider selection in user interfaces; provider configuration, auth, endpoints, and routing stay inside `rw-providers`.
